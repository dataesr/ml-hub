...
def _expectations_to_data(expectations: Dict[str, Any], format: Literal["json", "tsv"] = "json"):
    try:
        data = formatters.formatters_func[format](expectations["expected_response"])
        return data
...
def funders_similarity(pred: dict, gold: dict) -> float:
...
def projects_similarity(pred: dict, gold: dict) -> float:
...
def infrastructures_similarity(pred: dict, gold: dict) -> float:
...
def companies_similarity(pred: dict, gold: dict) -> float:
...
def eval_document(pred: dict, gold: dict, doc_id: str) -> dict[str, CategoryResult]:
    results = {}
...
def eval_row(outputs: dict, expectations: Any) -> dict[str, CategoryResult]:
    pred = outputs
    gold = _expectations_to_data(expectations, format="json")
    if pred is None or gold is None:
        logger.error("Failed to parse outputs or expectations (empty)")
        return {cat: CategoryResult(category=cat) for cat in CATEGORIES}
    return eval_document(pred, gold, doc_id=expectations.get("doc_id", "unknown_doc"))
...
def build_acknowledgement_scorers() -> list[Scorer]:
    ...
    for category in CATEGORIES:
        ...
        @scorer(name=f"{category}/precision")
        def _precision(outputs, expectations, category) -> float:
            return round(eval_row(outputs, expectations)[category].precision(), 4)

        @scorer(name=f"{category}/recall")
        def _recall(outputs, expectations, category) -> float:
            return round(eval_row(outputs, expectations)[category].recall(), 4)

        @scorer(name=f"{category}/f1")
        def _f1(outputs, expectations, category) -> float:
            return round(eval_row(outputs, expectations)[category].f1(), 4)

        @scorer(name=f"{category}/field_accuracy")
        def _field_accuracy(outputs, expectations, category) -> float:
    ...
    @scorer(name="overall/micro_f1")
    def _micro_f1(outputs, expectations) -> float:
    ...
    @scorer(name="funders/grant_id_recall")
    def _grant_recall(outputs, expectations) -> float:
